...
def audit(workflows: Path) -> dict:
    ...
    for path in paths:
        ...
        for child, times in notices.items():
            ...
            counts["repeat_notices"] += len(times) - 1
            first = min(times)
            ...
            chunk = candidates[0]
            if expected and (
                (chunk.get("context_id"), chunk.get("context_epoch")) != expected
            ):
                counts["first_chunk_identity_mismatch"] += 1
                continue
