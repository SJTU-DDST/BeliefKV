...
class LocalFrontierFeatures:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("project tool duration must be finite and non-negative")
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "LocalFrontierFeatures":
        return cls(
            ...
            project_class_duration_median_ms=(
                ...
                if raw.get("project_class_duration_median_ms") is not None else None
            ),
...
class FrontierBeliefModel:
                ...
                if (
                    ...
                    if delay is not None and _target_eligible(label, "external_wait"):
                        ...
                        if (
                            ...
                            project_errors.append(abs(
                                float(delay)
                                - local_features.project_class_duration_median_ms
                            ))
                            project_workflows.add(str(row.get("workflow_id") or ""))
        ...
        *,
        ...
        for row in values:
            ...
            for raw_features in row.get("invocations", ()):
                ...
                if (
                    ...
                    project_errors_by_workflow[workflow].append(abs(
                        float(label["next_boundary_delay_ms"])
                        - features.project_class_duration_median_ms
                    ))
                boundary = _normalize_boundary(label.get("next_boundary_kind"))
        ...
        if self.tool_feature_contract == "observed_command_child_project_v3":
            ...
            if len(grouped_errors) >= 8:
                ...
                self.project_error_p90_ms = max(
                    self.project_error_p90_ms or 0.0, grouped_errors[index]
                )
            else:
                self.project_error_p90_ms = None
            observation_counts["project_workflows_calibrated"] = len(grouped_errors)
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "FrontierBeliefModel":
        ...
        project_error = raw.get("project_error_p90_ms")
    ...
    *,
    ...
    return LocalFrontierFeatures(
        ...
        project_class_duration_median_ms=(
            ...
            >= 16
            and type(trigger_attributes.get("project_class_duration_median_ms"))
            in (int, float) else None
        ),
